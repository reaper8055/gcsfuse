...
// It returns the number of directories that were identified as empty but failed to be removed
// (e.g. due to race condition with file creation).
func RemoveEmptyDirsWithLocker(dir string, locker DirLocker) uint64 {
	_, contentionCount := removeEmptyDirs(dir, locker)
...
//  2. uint64: count of contention events (failed removals of "empty" dirs).
func removeEmptyDirs(dir string, locker DirLocker) (bool, uint64) {
	entries, err := os.ReadDir(dir)
	if err != nil {
	...
	for _, entry := range entries {
		if entry.IsDir() {
			...
			childEmpty, childContention := removeEmptyDirs(fullPath, locker)
	...
	return isEmpty, contentionCount
}
